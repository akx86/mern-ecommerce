...
import { useDispatch } from 'react-redux'; // 1. عشان نحدث حالة الدخول
import { useForm, Controller } from 'react-hook-form'; // 2. عشان الفالديشن
import { zodResolver } from "@hookform/resolvers/zod"; // 3. ربط zod بالفورم
...
import { loginSuccess } from '@/store/slices/authSlice'; // أكشن تسجيل الدخول
...
const registerSchema = z.object({
    ...
    path: ["passwordConfirm"], // الخطأ هيظهر تحت حقل التأكيد
});
...
const Register = () => {
    ...
    const dispatch = useDispatch(); // هوك الريدكس
    const fileInputRef = useRef(null);
    
    // States للصورة فقط (مش محتاجة validation معقد)
    ...
    const onSubmit = async (data) => {
        ...
        try {
            ...
            dataToSend.append('passwordConfirm', data.passwordConfirm); // لو الباك اند محتاجه
            ...
            // 3. Auto-Login Logic (الحتة السحرية) ✨
            // الباك اند بيرجع التوكين واليوزر في الريسبونس بتاع التسجيل
            dispatch(loginSuccess(response)); 
            
            toast.success(`Welcome, ${response.data.user.name}! 🎉`, { id: toastId });

            // 4. التوجيه للصفحة الرئيسية مباشرة
            setTimeout(() => {
                navigate('/'); 
            }, 500);
