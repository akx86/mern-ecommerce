...
import AdminLoading from '@/components/layout/AdminLoading';
import SalesChart from '@/components/layout/SalesChart';

// استدعاء مكونات Shadcn Select
import {
    Select,
    SelectContent,
    SelectItem,
...
const AdminDashboardPage = () => {
    ...
    // 1. State للتحكم في مدة الرسم البياني
    const [dateRange, setDateRange] = useState("7days");

    // 2. كويري الإحصائيات (يعتمد على dateRange)
    const { data, isLoading, isError } = useQuery({
        queryKey: ['admin-dashboard', dateRange], // أي تغيير في التاريخ هيعيد طلب البيانات
        queryFn: () => getDashboardStats(dateRange), // بنبعت القيمة للباك اند
        keepPreviousData: true // يحافظ على البيانات القديمة لحد ما الجديدة تيجي (عشان الصفحة مترمش)
    });

    // 3. كويري أحدث الطلبات (ثابت لا يتأثر بالفلتر)
    const { data: ordersData, isLoading: isLoadingOrders } = useQuery({
        queryKey: ['recent-orders'],
        queryFn: () => getAllOrders({ limit: 5, sort: '-createdAt' }), 
    });
    
    // شاشة التحميل (فقط في أول مرة)
    if (isLoading) {
        return <AdminLoading />
    }
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {statCards.map((card, index) => (
                    <motion.div
                        ...
                        className="relative overflow-hidden bg-slate-900/40 backdrop-blur-xl border border-white/10 p-6 rounded-2xl group hover:border-white/20 transition-all duration-300"
                    >
                        {/* Glow Effect */}
                        ...
                        <div className="flex justify-between items-start relative z-10">
                            ...
                            <div className={`p-3 rounded-xl bg-gradient-to-br ${card.color} shadow-lg shadow-black/20 group-hover:scale-110 transition-transform duration-300`}>
                        ...
                        <div className="mt-4 flex items-center gap-2 text-xs relative z-10">
                            ...
                            <span className="text-zinc-500">{card.desc}</span>
                        </div>
                    </motion.div>
                ))}
            </div>

           {/* 3. Bottom Section: Charts & Recent Activity */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-8">

                {/* Left Side: Sales Chart Area */}
                <div className="lg:col-span-2 bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-lg font-bold text-white">Sales Analytics</h3>
                        
                        {/* Shadcn Select Dropdown */}
                        <Select value={dateRange} onValueChange={setDateRange}>
                            <SelectTrigger className="w-[140px] bg-black/30 border-white/10 text-zinc-400 h-8 text-xs outline-none focus:ring-0 focus:ring-offset-0 focus:border-indigo-500/50">
                ...
                <div className="bg-slate-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6">
                    ...
                    <div className="space-y-4">
                        ...
                        {recentOrders.map((order) => (
                            <div key={order._id} className="flex items-center gap-4 p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors cursor-pointer group">
                                <div className="w-10 h-10 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 font-bold text-xs group-hover:scale-110 transition-transform">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-zinc-400 truncate">
                                        ID: #{order._id.slice(-6)} • {order.orderItems?.length || 0} items
                                    </p>
                                </div>
