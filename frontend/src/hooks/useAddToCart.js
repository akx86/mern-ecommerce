...
export const useAddToCart = () => {
    ...
    // التعديل 1: إضافة معامل showToast وقيمته الافتراضية true
    // ده بيسمح لنا نستخدم الهوك في أي مكان، ونقرر هل عايزين رسالة نجاح ولا لأ
    const handleAddToCart = async (product, quantity = 1, showToast = true) => {
        if (!product) return;

        const pId = product._id || product.id;
        
        const rawStock = product.countInStock ?? product.stock ?? product.product?.countInStock ?? 0;
        ...
        if (isAuthenticated) {
            ...
            // بنعمل return عشان نقدر نستخدم await في الصفحة
            return dispatch(addToCartAPI({ productId: pId, quantity }))
                .unwrap()
                .then(() => {
                    // التعديل 2: اظهار رسالة النجاح فقط لو showToast = true
            ...
            try {
                ...
                // التعديل 3: التحكم في التوست للزائر برضه
