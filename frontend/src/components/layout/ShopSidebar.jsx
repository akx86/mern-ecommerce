...
const ShopSidebar = () => {
  ...
  return (
    // [MODIFIED] Container: Dark Glass Style
    <aside className="hidden md:block w-[280px] sticky top-24 h-fit p-6 border border-white/5 rounded-2xl bg-slate-900/40 backdrop-blur-xl shadow-2xl shadow-black/20 transition-all duration-300">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <Button 
            ...
            // [MODIFIED] Reset Button Colors
            className="text-red-400 hover:text-red-300 hover:bg-red-500/10 text-xs transition-colors"
        >
          Reset All
        </Button>
      </div>
      ...
      <div className="relative mb-6 group">
        ...
        <Input 
          ...
          // [MODIFIED] Input Style: Dark & Glassy
          className="pl-9 bg-slate-950/50 border-white/10 text-slate-200 placeholder:text-slate-600 focus-visible:ring-indigo-500/50 hover:bg-slate-900/80 rounded-xl transition-all"
              ...
              {isLoading ? (
                ...
                categories.map((cat) => (
                  <div key={cat._id} className="flex items-center space-x-3 group">
                    ...
                    <label 
                      htmlFor={cat._id} 
                      className="text-sm font-medium cursor-pointer text-slate-400 group-hover:text-white transition-colors"
                    >
                      {cat.title}
                    </label>
                  </div>
        ...
        <AccordionItem value="price" className="border-none">
          ...
          <AccordionContent className="pt-4 px-1">
            ...
            <Slider
              ...
              // [MODIFIED] Slider styling is usually handled by global theme, 
              // but ensure your theme config uses Indigo for primary color.
